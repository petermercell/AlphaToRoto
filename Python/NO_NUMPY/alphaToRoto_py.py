...
def _get_bezier_basis(samples_per_bezier):
    """Return a cached list of N Bernstein basis tuples (b0, b1, b2, b3)
    evaluated at t = 1/N, 2/N, ..., 1.0 (i.e. the same sample points as the
    reference Python implementation -- start point is contributed by the
    previous segment, so we skip t=0).

    Each tuple satisfies b0 + b1 + b2 + b3 == 1 (within float round-off).
    """
...
def _flatten_shape_to_polyline(segments, samples_per_bezier=16):
    ...
    Pure-Python implementation: each cubic segment is evaluated against the
    cached Bernstein basis with an inline 4-term dot product per sample.
    """
    if not segments or segments[0][0] != 'move':
        return []
    ...
    for seg in segments[1:]:
        ...
        if kind == 'curve':
            _, c1x, c1y, c2x, c2y, ex, ey = seg
            # Inline cubic-bezier evaluation against precomputed basis.
            # Each basis row is (b0, b1, b2, b3) summing to 1.
            for (b0, b1, b2, b3) in basis:
                x = b0 * prev_x + b1 * c1x + b2 * c2x + b3 * ex
                y = b0 * prev_y + b1 * c1y + b2 * c2y + b3 * ey
                result.append((x, y))
            prev_x, prev_y = ex, ey
        elif kind == 'line':
            _, ex, ey = seg
...
def _cyclic_align(curr, prev):
    ...
    at lag `offset`. We return the offset at the peak.

    Pure-Python implementation: direct O(K^2) cross-correlation. At typical
    K (128..512) this runs in well under a millisecond per call; at the
    K=2048 worst case it's around 60ms per call, which is still acceptable
    given typical track counts.
    """
    K = len(curr)
    ...
    # Cache locally to avoid attribute lookups in the inner loop.
    curr_local = curr
    prev_local = prev
    ...
    for offset in range(K):
        s = 0.0
        # sum_i curr[(i+offset) % K] . prev[i]
        for i in range(K):
            cx, cy = curr_local[(i + offset) % K]
            px, py = prev_local[i]
            s += cx * px + cy * py
        if s > best_corr:
            best_corr = s
    ...
    if best_offset == 0:
        return list(curr)
    return [curr_local[(i + best_offset) % K] for i in range(K)]
